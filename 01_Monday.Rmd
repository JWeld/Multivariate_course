...
In this first chunk of code we load the R packages we will be using today
...
#### [2.2 Which metric corresponds to a geographic distance, and which to a species community distance?]{style="color:forestgreen"}

```{r message=FALSE, warning=FALSE}
# create a  data set with 2 columns (x and y) and 4 rows
#
...
#### [3.1 Why is there a difference between the two plots in "PCA plot 4" ? Which one is best to use?]{style="color:forestgreen"}
...
sort(abs(loadings[,1]), decreasing = TRUE) # Loadings axis 1
sort(abs(loadings[,2]), decreasing = TRUE) # Loadings axis 2

```

As you saw in the lecture, you can also run approximate significance tests for a PCA. You can do this with e.g. the BiodiversityR package, but unfortunately we are unable to install that in the Posit Cloud environment (because it requires installing things in the underlying operating system as well as in R). We can anyway draw an Equilibrium Contribution Circle, by adding the equations for the circle in our code. This defines a region beyond which variables can be interpreted with greatest confidence.

#### [4.2 Which variables are unimportant for the two-dimensional projection of your sites along axis 1 and 2?]{style="color:forestgreen"}

```{r, warning=FALSE, message=FALSE}
# Equilibrium Circle 
n <- nrow(env.pca$CA$u) #take n and p from the fitted object rather than the source dataframe
p <- nrow(env.pca$CA$v)
const <- ((n - 1) * env.pca$tot.chi)^0.25
radi  <- sqrt(2 / p) * const

sc  <- scores(env.pca, display = c("sites", "species"), scaling = 1, choices = 1:2)
lim <- range(c(unlist(sc), -radi, radi))

biplot(env.pca, scaling = "sites", xlim = lim, ylim = lim) #can only draw circle on scaling 1
circ <- seq(0, 2 * pi, length.out = 361)
lines(cbind(cos(circ), sin(circ)) * radi, col = "blue")
