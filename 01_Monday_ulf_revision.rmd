...
library(tidyverse)
library(corrplot)
library(BiodiversityR) # JAMES: New package added /Ulf
```
...
Now we can do a simple linear regression, using variables x and y in the dataset called df. The results are stored in the object "reg".
...
#### [3.1 Is there any way to see if a PCA plot is based on scaled or unscaled data, just by looking at the plot (i.e. compare plot 2 and 3)?]{style="color:forestgreen"}

#### [3.2 What is the difference between the two plots in "PCA plot 4" ? Which one is best to use?]{style="color:forestgreen"}
...
## PCA plot 6:
# Same as plot 5, but without species for better visual inspection of the sites
par(mfrow = c(1,2))
...
### [Exercise 4: Significance test for the axes in the PCA in Question 3]{style="color:cornflowerblue"}
...
#### [4.1 Look at the loadings, i.e. the effects by the different variables for axis 1 and axis 2. Which variables are most important?]{style="color:forestgreen"}

#### [4.2 Continue with an approximate significance test of the variables. Which variables are important for the ordination?]{style="color:forestgreen"}

#### [4.3 Finally, have a look at the indicative significance of each PCA axis, i.e. compare explained variation in the data, with explained variation in a null model (broken-stick). What does the result tell you? Is the PCA on the Dune Meadow explanatory data reliable?]{style="color:forestgreen"}

```{r, warning=FALSE, message=FALSE}
# Start to look at the effects by individual variables
...
# Then look at the significnace of the variables
env.plot <- ordiplot(env.pca, type = "none", scaling=1)
points(env.plot, "sites", pch=19, col="red")
points(env.plot, "species", pch=4, col="red")
text(env.plot, "species", col="blue", cex=0.9)
ordiequilibriumcircle(env.pca, env.plot)

# Finally, test if the ordination axes are worth interperting
sig <- PCAsignificance(env.pca, axes = 7)
barplot (sig[c("percentage of variance", "broken-stick percentage"), ], beside = T,
         xlab = "PCA axis", ylab = "Explained variation [%]", col = c('grey', 'black'), legend = TRUE)
