...
load("C:/Users/hhen0001/Documents/Multivariate Statistics/R conversion/MVM_data.RDS")
...
#### <span style="color:forestgreen">What happens with the eigenvalues, total variation, and ordination diagram?</span>     
...
You may be surprised that you haven't get any total inertia values when printing decorana results, 
...
inertia values (also, it produces only four axes, i.e. four eigenvalues). However, you may get total 
inertia by applying correspondence analysis (CA) on your data.
...
#### <span style="color:forestgreen"> Compare total variation, eigenvalues and ordination diagrams between exercises 5, 6 and 7! Explain the differences! What is the effect of making a species supplementary?</span>
...
#### <span style="color:forestgreen"> What happens with the eigenvalues, variation, and ordination diagram? Why did we choose to add Chenopodium album in the new sample? </span>
...
#### <span style="color:forestgreen"> What happens with the eigenvalues and variation? Look at the ordination diagram. Where is the extra sample located? </span>
...
#### <span style="color:forestgreen"> What is the effect of downweighting? Compare eigenvalues, total variation and ordination diagrams from Exercise 5 and 10, and explain the differences! </span>
...
#### <span style="color:forestgreen"> Compare the ordination diagrams and numerical results! What does the significance test tell you? When only looking at the results, which method explains the variation best? Taking the nature of the data into account, which method should be used? What is meant explained variation and adjusted explained variation? </span>
...
### <span style="color:cornflowerblue">Question 14:	Use the results from the CCA ordination in exercise 13 to look at and interpret the variance inflation factor.   </span>
...
#### <span style="color:forestgreen"> Inspect and interpret the results of the permutations and all graphs! Compare with the CCA on the full set of explanatory data in exercise 13. </span>
...
ordiplot(dune.ccaMN, type="text", main="CCA only Management")
...
#### <span style="color:forestgreen"> We have now concluded that management type is of subordinate importance. Now, you want to test the importance or significance of individual environmental variables. This is done by Forward selection. What is the effect of Forward selection? In what way do these results differ from the results obtained when the whole Environmental data table was used as explanatory variables? Were the set of explanatory variables selected by the Forward selection the best set? What is the effect of P value correction, and why is it performed?  </span>

```{r ex20, inclue=TRUE, warning=FALSE, message=FALSE}

mod0 <- cca(dune ~ 1, dune.env)  # Model with intercept only
mod1 <- cca(dune ~ ., dune.env)  # Model with all explanatory variables

## With scope present
FS <- ordistep(mod0, scope = formula(mod1), direction="forward")
FS$anova

## Holm correction on the p-values

#pval.adj <- p.adjust(FS$pval, method = 'holm', n = n.tests)
