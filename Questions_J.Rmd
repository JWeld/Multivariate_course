...
First we need to import the data we will be using. This is included in the "vegan" package but we will be using a slightly modified and extended version.
...
## Quick code to replace missing values with the column mean, and save the results as a new dataframe
## called dune.mean.ell.impute.
...
plot(time.cca, display = c("lc"))

#plot(time.cca, dis=c("wa","lc"))
#ordispider(time.cca, groups = treat, label = T)
...
#baci <- cca(SpeTS,EnvTS)

summary(baci)
#baci
#plot(baci)
plot(baci, dis=c("lc","bp"))
