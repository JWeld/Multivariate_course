...
(iii) Can you find which explanatory variables that are most important for the responsevariable, looking at the loading plot?
(iv) Does the data seemed to be in need of a transformation? </span>
...
#The loading plot is often used for interpretation purposes, for instance to look for known spectral peaks or profiles:  #S: I think this is mainly true when you use PLS on other type of data, in this case it doesn´t make any sense (on this type of data). 
#What we would want is to have the loading.weight plot as a scatter plot (like the scoreplots below),
#with the labels of each variable on top of the loadings, that's what you use to look for patterns.
#I think PLS was originally created to look at spectral data and then this below makes sense, but not here
plot(pls.fit2, "loadings", comps = 1:8, legendpos = "topright", xlab = "nm")
...
#The scores represents the different lakes, so it doesn't make sense to label them with the varibale names
plot(pls.fit2, plottype = "scores", comps = 1:3,labels = names(coefficients))
