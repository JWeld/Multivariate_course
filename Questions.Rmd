...
This exercise is only to get used to working with R Notebook/R Markdown.
The data in this first exercise are not multivariate and the statistical
analysis is a simple linear regression. All following exercises will
start with this kind of introduction.

### [For each exercise there will be a number of explicit questions written in green.]{style="color:forestgreen"}

To pass the course, you should give your answers to these questions to
one of the teachers during the computer labs.
...
These terms are often used interchangeably to mean a combination of R
code, results, and formattable text, such as this one. They allow you to
present your results, the code used to generate the results and your
interpretation and comments all together in one document that you can
easily update, and which is easy for a reader to understand (R Notebooks
are a minor update of R Markdown files that allow for easier editing as
you can run just part of your code and see the results in a preview.
Both notebook and markdown are .Rmd files, and are written in exactly
the same way, so don't worry about the difference for this course!).

------------------------------------------------------------------------

#### Example of a simple analysis

First we load or create the data we will be using. The box below is an
...
So I could write my notes here instead, which is probably easier to read
for anything more than a few words of explanation.

Now we can do a simple linear regression. The results are stored in the
object "reg".
...
The "dune" dataset we will use is included in the vegan package (an
extensive collection of tools for multivariate analysis), but we will
also be using some slightly modified and extended versions of it, which
we will load as needed. We will also load some other packages to extend
the range of analyses available to us.
...
Here, we will create a simple data set consisting of 2 columns of
measurement made at 3 sites we will then calculate a series of distance
metrics using the vegdist() function.

#### [Questions: 2.1 How do the metrics differ? 2.2 Which metric corresponds to a geographic distance? 2.3 Where would one find the formula for the Mahalanobis distance?]{style="color:forestgreen"}
...
Compare the plots on the PCA with and without scaling (PCA plot 4) and
discuss the differences.

#### Questions: 3.1 Is there any way to see that one is on scaled data and one is not? 3.2 What is the difference between PCA plot 4 and PCA plot 5? Which one is best to use? 3.3 Discuss the difference between the plots in plot PCA plot 6 and 7! {style="color:forestgreen"}
...
#### [Question: 5.1 Look at the eigenvalues, the length of gradient, the total variation and the ordination diagram. Explain the differences between results from CA and DCA.]{style="color:forestgreen"}
...
### [Question 6: Add one new species with cover degree 9 at the site with the lowest number of species. Do a new DCA-ordination.]{style="color:cornflowerblue"}

#### [Question: 6.1: What happens with the eigenvalues and ordination diagram?]{style="color:forestgreen"}
...
You may be surprised that you haven't got any total inertia values when
printing decorana results, although in other software (e.g. CANOCO)
these are available, together with percentage variance explained by
particular axes. The reason for this is that DCA does not support the
concept of total inertia values (also, it produces only four axes, i.e.
four eigenvalues). See the comment in the code for the question 5.
...
### [Question 7: Effects from increased variation.]{style="color:cornflowerblue"}

Add one new sample to the data, then add a cover of 9 for the existing
species Chenopodium album "Chealbu" in the new sample. Make a new
DCA-ordination. <br> Next, remove the Chealbu = 9, and add instead a
cover of 5 for the exisiting species Juncbufo and make a new DCA.

#### [Questions: 7.1 What happens with the eigenvalues and ordination after adding Chealbu and after adding Juncbufo? 7.2 Why did we choose to add these two species in the new sample?]{style="color:forestgreen"}
...
NewSampleJunc <- c(rep(0,15), 5, rep(0,10))
...
### [Question 8: Principal Coordinates Analysis (PCoA)]{style="color:cornflowerblue"}

Principal Coordinates Analysis (PCoA), also known as Multidimensional
Scaling (MDS), is a generalization of the ordination techniques we have
already seen as it can be estimated with any distance matrix. In
contrast, PCA is restricted to Euclidean distances while CA preserves
Chi-Square distances. Here, we will explore PCoA on four different
distance matrices: (i) Bray-Curtis (ii) Jaccard (iii) Euclidean (iv)
Mahalanobis

The R base statistics library offers the routine cmdscale() for
performing MDS. There are numerous other libraries offering PCoA
analysis

#### [Questions: 8.1 How similar are the ordination plots based on the four distance matrices?(remember question 2!) 8.2 Why are some plots more similar than others? 8.3 Which PCoA plot is most similar to the output from a PCA? 8.4 Which PCoA plot is most similar to the output from a CA?]{style="color:forestgreen"}
...
### [Question 9: Non Metric Multidimensional Scaling]{style="color:cornflowerblue"}

NMDS does not use the absolute abundances of species in communities, but
rather their *rank orders*. The use of ranks omits some of the issues
associated with using absolute distance (e.g., sensitivity to
transformation). At the conclusion of this exercise, you should be able
to: State which metrics are supported in the vegan implementation of
NMDS. Evaluate the appropriate number of dimensions for the ordination.
Evaluate the consequences of using different distance metrics.

#### [Question: 9.1 Compare the vegan default NMDS to the results of a CA and a DCA.]{style="color:forestgreen"}
...
### [Question 10: Data Quality Assessment]{style="color:cornflowerblue"}

These are only a few examples on how to assess data quality.

#### [Questions: 10.1 Is there a pattern in the distribution of missing values that may lead to the omission of a species or a sample? 10.2 Would you say that the original Dune Meadow data set is OK for further analyses? Motivate your answer!]{style="color:forestgreen"}
...
<br> We can also try testing for outliers in the data. Be aware that
this is only indicative, and you still need to exercise your judgement
about what should be excluded as an outlier!
...
plot(stats::density(d1mean), col = "red", xlab="Distance between observations", main = "Indicative test of multivariate outliers\nData with noise")
...
### [Question 11: Do an RDA and a CCA on the Dune Meadow data set, using the full set of environmental variables.]{style="color:cornflowerblue"}

#### [Questions: 11.1 Compare the ordination diagrams and numerical results! What does the significance test tell you? 11.2 When only looking at the results, which method explains the variation best? 11.3 Taking the nature of the data into account, which method should be used?]{style="color:forestgreen"}
...
#### [Question: 12.1 Look at and interpret the inflation factors for each environmental variable.]{style="color:forestgreen"}
...
rule is that values over 10 indicate redundant constraints.
...
#### [Questions: 13.1 Compare the constrained and unconstrained ordinations! What is the difference between the methods? 13.2 When could an unconstrained ordination with explanatory variables be a good option?]{style="color:forestgreen"}
...
#### [Questions: 14.1 What is the main difference between the two ways. How to know which method to select? 14.2 What does the display = "cn" in the last plots mean?]{style="color:forestgreen"}
...
regimes (SF standard farming, BF biodynamical farming, HF hobby farming,
and NM nature management) have any effect on the species distribution.
This is done by using these four management types as environmental
variables in a constrained ordination.

#### [Question: 15.1 Inspect and interpret the results of the permutations and graph! Compare with the CCA on the full set of explanatory data in exercise 13.]{style="color:forestgreen"}

To investigate if the vegetational pattern is an effect of management,
or if the pattern is random, Monte Carlo permutations will be used.
...
To do this, do another CCA with A1 and Moisture as environmental
variables and perform a significance test.

#### [Question: 16.1 Explain the results of the permutations! Compare the results in exercises 16 and 17!]{style="color:forestgreen"}
...
Now, you have concluded that both soil characteristics and management
type are determining the species composition significantly. But, what if
the observed differences between management types are not caused by
management type but by initial differences in soil characteristics? To
investigate this, you have to test whether there still is a difference
in vegetation between management types, after accounting for (i.e.
removing) the effects of soil characteristics. This is done by a partial
CCA ordination. Partial ordinations are used to eliminate effects of
selected variables by specifying them as covariates. In this case, use
the two soil characterising variables (A1 and Moisture) as covariates,
and the four management types as environmental variables.

#### [Questions: 17.1 What does the permutation test tell you? 17.2 Compare the results with the results from the permutation test on only management types]{style="color:forestgreen"}
...
The amount of applied manure is a third important variable given in the
environmental data set associated with the dune meadow data set. Repeat
exercise 17, but with manure added to the set of covariates.

#### [Questions: 18.1 What was the effect of adding manure to the set of covariates? 18.2 We may expect that Management and Manure are correlated, check visually by making a boxplot of Manure vs Management type using the R code: boxplot(Manure\~Management, data = dune.env.original). What does the result tell you?]{style="color:forestgreen"}
...
ordihull(MN.cca2,groups = dune.env.original$Management,draw = "polygon",col = "grey70", label = T)
...
We have now concluded that management type is of subordinate importance.
Now, you want to test the importance or significance of individual
environmental variables. This is done by Forward selection.

#### [Questions: 19.1 What is the effect of Forward selection? 19.2 In what way do these results differ from the results obtained when the whole Environmental data table was used as explanatory variables? 19.3 Were the set of explanatory variables selected by the Forward selection the best set? 19.4 Why would one perform a p-value correction?]{style="color:forestgreen"}
...
Since there is (a potentially high) number of tests of significance
during the forward selection procedure, it is better to apply a
correction for multiple testing issue. Here we apply the Holm
correction.
...
### [Question 20: Interpreting ordination results using species traits]{style="color:cornflowerblue"}
...
#### [Questions: 20.1 Which Ellenberg values are most important for the distribution of the species? 20.2 What do the different axes represent in terms of environmental gradients?]{style="color:forestgreen"}
...
### [Question 21: Decomposition of variance]{style="color:cornflowerblue"}
...
#### [Questions: 21.1 Perform the analyses and interpret the results - how much of the total variation (% of All) is explained by: - uniquely by Management (effect of Soil removed), - uniquely by Soil (effect of Management removed), - jointly by Soil and Management (the interaction) - Not by Soil or Management?]{style="color:forestgreen"}
...
### [Question 22: Analysing a time series with vegetation data]{style="color:cornflowerblue"}
...
#### [Questions: 22.1 Is there a significant change in species composition over time? 22.2 What is the effect of defining Plots as cofactors?]{style="color:forestgreen"}
...
time.cca
treat <- EnvTS$Time #
colvec <- c("red2", "blue") #set colours to be applied to different levels of factor "Time"
...
time2.cca
#permutation test
with(EnvTS, anova(time2.cca, by="term", perm=100))
```
...
### [Question 23: A multivariate Before-After-Control- Impact (BACI) study]{style="color:cornflowerblue"}
...
#### [Questions: 23.1 Did the treatment have an effect? 23.2 What was permuted?]{style="color:forestgreen"}
...
### [Question 24: Comparing ordinations]{style="color:cornflowerblue"}
...
repeated samplings of the same objects using the same descriptors.

#### [Questions: 24.1 How similar is the arrangement of sites in ordination space based on plant and insect data? 24.2 Do the three analyses (Procrustes, Co-Correspondence and Mantel) indicate similar ordinations of sites based on plant vs. insect data?]{style="color:forestgreen"}
...
ca.site.scores<-vegan::scores(bugs.ca, c(1,2), display = "sites") # Extract the site scores from the CA (axis 1 and2)
dune.env.bugs<-cbind(dune.env, ca.site.scores) # Add the site scores to the dune.env
dune.bugs.cca<-cca(dune, dune.env.bugs) # Make a CCA on the plant data with CA axis 1 and 2 as additional environmental variables
dune.bugs.cca
anova(dune.bugs.cca)
plot(dune.bugs.cca)
```
...
### [Question 25 : Classification 1]{style="color:cornflowerblue"}
...
#### [Questions: 25.1 How many clusters seem to exist in the Dune data set? 25.2 Is a 3- or 4-cluster solution a better match to the management type in the dune.env dataset? 25.3 Do the two approaches to hierarchical clustering reveal similar patterns in the Dune data set? 25.4 Interpret the Hopkins statistic for the Dune data set.]{style="color:forestgreen"}
...
### [Question 26: Classification 2: Partition and Regression Trees]{style="color:cornflowerblue"}
...
#### [Questions: 26.1 Based on a visual analysis, which dimensions provide the best separation between iris species? 26.2 Are your insights from the visual analysis confirmed by the partition tree analysis? 26.3 Does the partition tree provide a perfect species identification or are there misclassifications?]{style="color:forestgreen"}
...
### [Question 27: Principal Response Curves]{style="color:cornflowerblue"}
...
#### [Questions: 27.1 Was there an effect of dose over time on the invertebrate community? 27.2 Which species were most affected, i.e changed most under the highest dose compared to the control ?]{style="color:forestgreen"}
...
### [Question 28: Differences between groups. ANOSIM, adonis and SIMPER]{style="color:cornflowerblue"}
...
#### [Questions: 28.1 What do the results from anosim and adonis tell you? 28.2 Give one other example of predefined groups that could be uses as the grouping variable(s)!]{style="color:forestgreen"}
...
with(dune.env.original, points(dune.ca, display = "sites", scaling = "symmetric", col = as.numeric(Management), pch=16))
with(dune.env.original, ordispider(dune.ca, Management, scaling = "symmetric", col="skyblue"))
with(dune.env.original, ordihull(dune.ca, Management, scaling = "symmetric",label = TRUE))

dune.dist <- vegdist(dune, method = "bray") #create distance matrix based on Bray-Curtis method
dune.ano <- anosim(dune.dist, dune.env.original$Management) #Comparing groupings based on management
summary(dune.ano)
...
SIMPER (Similarity Percentage) will do this for you. The test does not
come with significance testing. In the output tables, taxa are sorted in
descending order of contribution to group difference.

#### [28.3 Compare the different groups and check which species contribute mostly to the difference between pairs of clusters.]{style="color:forestgreen"}
...
indicative for the four different management types. Use the resulting
data table to find the two top indicator species for each of the four
Management types!
...
### [Question 29: DFA]{style="color:cornflowerblue"}
...
#### [Questions: 29.1 How well does the algorithm distinguish between the species? 29.2 Here our groups are pre-defined (species). Would it be OK to use one of the clustering methods to find groups before running the DFA?]{style="color:forestgreen"}
...
<br> LDA determines group means and computes, for each individual, the
...
### [Question 30: Model based ordination]{style="color:cornflowerblue"}
...
#### [Question: 30.1 How do the ordinations produced compare to a distance based ordination (for example an NMDS) of the same species data?]{style="color:forestgreen"}

```{r}
library(gllvm) #Generalised linear latent variable models
library(readxl)
#load species abundance data
spe<-as.data.frame(read_xlsx("dune.xlsx", na = ".",sheet = 1, col_names = TRUE)); spe[1]<-NULL
...
```{r}
par(mfrow = c(1,2))
ordiplot.gllvm(fitord, biplot = TRUE, s.colors = "blue", spp.colors = "grey50", alpha = 0.4, cex.spp = 0.9, jitter = T)

abline(h = 0, v = 0, lty=2)
#compare with an NMDS
distord <- metaMDS(spe)
ordiplot(distord, type = "t")
par(mfrow = c(1,1))
```
...
### [Question 31: Fourth Corner]{style="color:cornflowerblue"}
...
#### [Question: 31.1 Do the correlations in the plot make sense ecologically?]{style="color:forestgreen"}
...
env<-as.data.frame(read_xlsx("dune_env.xlsx", na = "NA",sheet = 1, col_names = TRUE)); env[1]<-NULL
env$Management<-as.factor(env$Management)
env$Use<-as.factor(env$Use)
#This is from a database of plant traits
traits<-as.data.frame(read_xlsx("TylerTraits.xlsx", na = "NA",sheet = 1, col_names = TRUE, skip = 1))
...
#Fourth corner
library(mvabund)
#Given that there are many traits and even environmental factors that are not strong predictors of 
#abundance, we can reduce the number of predictor terms in our models.LASSO is an optimization algorithm (Least #Absolute Shrinkage and Selection Operator) that is used in this case.
...
plot.4th = levelplot(t(as.matrix(fitfourth$fourth.corner)), xlab="Environmental Variables",
...
### [Question 32: PLS]{style="color:cornflowerblue"}
...
#### [Questions: 32.1 Which explanatory variables are most important to explain the response variable (Biovolume)? 32.2 Are there any obvious outliers in the dataset? 32.3 Does the data seemed to be in need of a transformation?]{style="color:forestgreen"}
...
pls.response <- dplyr::select(dune.env, A1, Moisture) %>% as.matrix()
pls.exp <- as.matrix(dune)
pls.fit <- plsr(pls.response ~ pls.exp,
                na.action = na.omit, validation = "LOO")
...
pls.fit2 <-  plsr(pls.response ~ pls.exp, ncomp = best.dims, na.action = na.omit)
...
corrplot(pls.fit2, comps=1:3, labels = names(coefficients) )
...
plot(pls.fit2, ncomp = best.dims, asp = 1, line = TRUE)
