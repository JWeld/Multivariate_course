...
#### [13.2 When only looking at the numerical results, which method explains the variation best?]{style="color:forestgreen"}
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, inclue=TRUE}
...
### [Exercise 14: Use the results from the CCA ordination to look at and interpret the variance inflation factor, and to look for warnings about outliers in the explanatory data]{style="color:cornflowerblue"}
...
Function vif.cca gives the variance inflation factors for each constraint or contrast in factor constraints. In partial ordination, conditioning variables are analysed together with constraints. Variance inflation is a diagnostic tool to identify useless constraints. A common suggestion is that values over 10 indicate redundant constraints.
...
#### [15.1 What is the difference in how explanatory variables are added in constrained and unconstrained ordinations?]{style="color:forestgreen"}

#### [15.2 When could an unconstrained ordination with explanatory variables be a good option?]{style="color:forestgreen"}

```{r, inclue=TRUE, warning=FALSE, message=FALSE}
# CA with env. variables fitted afterwards
dune.ca <- ca(dune)
...
#### [17.1 Inspect and interpret the results of the significance test and graph! Compare with the CCA on the full set of explanatory data in exercise 15.]{style="color:forestgreen"}
...
#### [18.1 Explain the results of the permutations! Compare the results in exercises 17 and 18]{style="color:forestgreen"}
...
#### [19.1 What does the result of the permutation test tell you?]{style="color:forestgreen"}
...
The amount of applied manure is a third important variable given in the environmental data set associated with the dune meadow data set. Repeat exercise 19, but with manure added to the set of covariates.
...
We may expect that Management and Manure are related, check visually by making a boxplot of Manure vs Management. Let's have a look...
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, inclue=TRUE}
