...
# You can also use adonis to test more advanced models. Here is an example how the same model as in the partial CCA in exercise 17 can be tested by adonis.
dune.ado2<-adonis2(dune~Manure+Management, data = dune.env.original, permutations = 9999)
dune.ado2
# As in the partial CCA, management is non-significant and accounts for 14% of variation (alongside 36% for manure, and 50% residual)
...
We can take the output and convert it to a dataframe so we summarise the
most important information in all of this, and easily rearrange the
columns to show the best indicator for each group.

```{r}
#we convert the output to a dataframe and do more with it
...
dplyr::arrange(rownames_to_column(iva.df), desc(BF))
```
...
#We will limit the number of bootstrap reps to 5 (nBoot=5) but 1000 should be used for final analyses
