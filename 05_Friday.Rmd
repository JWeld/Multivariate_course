...
# pairwiseAdonis is not on CRAN, so install it from GitHub the first time only.
# remotes is a lightweight alternative to devtools for this purpose.
...
measure. In this case, we use management and change to Bray-Curtis
similarity index.
...
summary(dune.ano)
plot(dune.ano, xlab = "Anosim, Dune meadow management types") #note the warning message here:
...
dune.ado2<-adonis2(dune~Management*A1, by = "terms", data = dune.env.original)
dune.ado2
...
#arrange in descending order to find best indicator species, change HF to other management types as needed
...
#Calculates a cross-tabulation of predicted and observed classes with associated statistics.
...
# Remove traits not feasible for this analysis. Select by name rather than by
# column position, so the code still does the right thing if the trait file
# gains or loses columns. If you want something back in the model, just delete it
# from this list.
drop_traits <- c("Biodiversity_relevance", "Nectar", "Heat", "Cold", "Temp",
                 "Continentality", "N_fixation")
missing_drop <- setdiff(drop_traits, names(traits))
if (length(missing_drop) > 0) {
  warning("Not found in trait file: ", paste(missing_drop, collapse = ", "))
}
traits <- traits[, setdiff(names(traits), drop_traits), drop = FALSE]

# traitglm() matches species to traits by position, not by name, so the rows of
# traits must be in exactly the same order as the columns of spe. Check that the
# two files contain the same species, then reorder traits to match spe.
stopifnot(setequal(colnames(spe), rownames(traits)))
traits <- traits[colnames(spe), , drop = FALSE]
stopifnot(identical(rownames(traits), colnames(spe)))
...
fitfourth$fourth.corner #examine fourth corner terms, you may want to convert to a dataframe for further analysis
#View(as.data.frame(fitfourth$fourth.corner))
...
plot.4th = levelplot(
  ...
  scales = list(x = list(cex = 0.6, rot = 45), y = list(cex = 0.6))
)
print(plot.4th)
