...
```{r ex7, include=TRUE, warning=FALSE, message=FALSE}
...
### [Exercise 8: Effects from increased variation.]{style="color:cornflowerblue"}

Add one new sample to the data, then add a cover of 9 for the existing species Chenopodium album "Chenalbu" in the new sample. Make a new DCA-ordination. Next, remove the Chenalbu = 9, and add instead a cover of 5 for the existing species Juncbufo and make a new DCA.

#### [Questions:]{style="color:forestgreen"}

#### [8.1 What happens with the eigenvalues and ordination after adding Chenalbu and after adding Juncbufo?]{style="color:forestgreen"}
...
dune.hellinger.distance.matrix <- vegdist(dune, method = "hellinger")
...
dune.hellinger.location <- cmdscale(dune.hellinger.distance.matrix)
...
plot(dune.hellinger.location, xlab = "PCoA 1", ylab = "PCoA 2")
title(main = "PCoA on Hellinger distances of dune sites")
text(
  dune.hellinger.location,
  labels = row.names(dune.hellinger.location),
  pos = 2
)
```
...
#### [10.2 What are the consequences of changing the number of dimensions in an NMDS (i.e. from 2 to 3 dimensions)? Which is best for an NMDS on the dune meadow data, based on Bray-Curtis distance?]{style="color:forestgreen"}

#### [10.3 Compare the NMDS on the dune meadow data based on Bray-Curtis distances with the NMDS based on Euclidean distances.]{style="color:forestgreen"}
...
#### [11.2 Based on the multivariate outlier analyses, would you say that the original Dune Meadow data set is OK for further analyses? Motivate your answer!]{style="color:forestgreen"}
...
# Create some outliers and missing values in a temporary dataset called mvo.dune
...
dotchart(as.matrix(mvo.dune), main = "Cleveland plot", xlab = "Observed value")
...
# Function to detect multivariate outliers via mean distance analysis
detect_outliers <- function(data, title_suffix) {
  # Calculate pairwise Euclidean distances
  dist_matrix <- vegdist(data, method = "euclidean", na.rm = TRUE)
  
  # Extract mean distance for each observation
  dist_long <- as.matrix(dist_matrix)
  mean_dist <- rowMeans(dist_long, na.rm = TRUE)
  
  # Calculate standard deviation (threshold: > 3 indicates strong outliers)
  sd_val <- sd(mean_dist)
  ...
  plot(
    ...
    main = paste0("Indicative test of multivariate outliers\n", title_suffix)
  )
  legend("topright", paste("SD =", round(sd_val, 1)))
  
  return(sd_val)
}

# Apply to both datasets
sd_noisy <- detect_outliers(mvo.dune, "Data with noise")
sd_original <- detect_outliers(dune, "Original Dune Meadow data")
...
head(gf)#the values in column CA2 are the cumulative goodness of fit values over the two axes
