...
K = Continentality (i.e. preference for an oceanic or inland continental
climate), F = Moisture, R = Reaction (i.e. soil pH preference), N =
Nitrogen, S = Salinity.
...
dune.mean.ell.impute <- data.frame(sapply(dune.mean.ell,
                                          function(x)
                                            ifelse(is.na(x),
                                                   ...
                                                   x)))
#CCA analysis
#create global model with CCA (including all variables) and test it's significance, and if it is significant, then
...
- Group 1 is Management

- Group 2 is the soil variables (A1 and Moisture)

In ordinations, the variance is expressed by the sum of the eigenvalues.
...
```{r, warning=FALSE, message=FALSE}
#This partials out (removes) the effect of Plot before analysing the effects of Time
#First load the time series versions of the data
SpeTS <- readRDS("Data/SpeTS.RDS")
EnvTS <- readRDS("Data/EnvTS.RDS")
EnvTS$Plot <- as.factor(EnvTS$Plot)
EnvTS$Time <- as.factor(EnvTS$Time)
...
```{r}
#permutation test
with(EnvTS, anova(time.cca, by="term", permutations = 999, strata=Plot))
```
...
time2.cca
#permutation test
with(EnvTS, anova(time2.cca, by="term", permutations = 999))
```
...
#Here with() is a special function that makes the variables in EnvTS visible to
#the following command. If you only type Treat at the R prompt, you will get
#an error about a missing object
```
...
ordihull(
  ...
  col = "grey70",
  label = T
) #draw hulls around area of each level of factor "Treat"
```
...
#### [26.2 Do the two analyses (Procrustes, and Mantel) indicate similar ordinations of sites based on plant vs. insect data?]{style="color:forestgreen"}

```{r ex24, include=TRUE, warning=FALSE, message=FALSE}
#
...
We can also take the output from one ordination and use it as
explanatory variables in another ordination....
...
Start by testing the non-hierarchical K-means clustering. Note that it
is recommended that K-means need at least 100 observations (500
according to some sources) to be reliable! Let us ignore the sample size
issue for the moment, and ask for 4 clusters (for later comparison with
the four management types).

After the non-hierarchical K-means exercise, change to test hierarchical
methods. Try different combinations of agglomeration algorithms and
similarity indices. The code below is for all combinations of average
and Wards agglomeration and Euclidian and the Bray-Curtis similarity
measures.
...
# As K-means is based on Euclidean distances, we should hellinger transform our species data
dune.h <- decostand(dune, method = "hellinger")
...
necessarily get exactly the same results if you repeat it. There are
...
```{r}
#Euclidean distance and “average” agglomeration algorithm 
dend2 <- dune %>% # data
  dist(method = "euclidean") %>% # calculate a distance matrix, choose method
  hclust(method = "average") %>% # Hierarchical clustering, average method
  as.dendrogram # Turn the object into a dendrogram.
dend2 %>% set("labels_color") %>%  set("branches_k_color", k = 4) %>% plot(main = "Euclidean and average")
#
#Euclidean distance and “Ward’s” agglomeration algorithm 
dend3 <- dune %>% # data
  dist(method = "euclidean") %>% # calculate a distance matrix, choose method
  hclust(method = "ward.D") %>% # Hierarchical clustering, Ward’s method
  as.dendrogram # Turn the object into a dendrogram.
dend3 %>% set("labels_color") %>%  set("branches_k_color", k = 4) %>% plot(main = "Euclidean and Ward's")
...
dend4 <- dune.dist %>% # data (Bray-Curtis)
  hclust(method = "average") %>% # Hierarchical clustering, average method
  as.dendrogram # Turn the object into a dendrogram.
dend4 %>% set("labels_color") %>%  set("branches_k_color", k = 4) %>% plot(main = "Bray-Curtis, Average")
#
...
#
dend5 <- dune.dist %>% # data (Bray-Curtis)
  hclust(method = "ward.D") %>% # Hierarchical clustering, Ward’s method
  as.dendrogram # Turn the object into a dendrogram.
dend5 %>% set("labels_color") %>%  set("branches_k_color", k = 4) %>% plot(main = "Bray-Curtis, Ward’s")

```

Tanglegram is a function to compare dendrograms that you might want to
try out as an extra

```{r}
tanglegram(dend2, dend3, main = "Eucl. average vs. Ward")
tanglegram(dend4, dend5, main = "B-C average vs. Ward")
tanglegram(dend3, dend5, main = "Eucl. Ward vs. B-C Ward")
